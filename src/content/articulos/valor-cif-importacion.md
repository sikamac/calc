---
title: "Valor CIF en importaciones: qué es y cómo se calcula"
date: 2026-06-05
updatedDate: 2026-06-09
category: "Costos"
image: "../../assets/images/guides/valor-cif.png"
description: "Aprendé qué significa el valor CIF, qué componentes incluye y por qué es clave para calcular impuestos de importación."
author: "Equipo de GIST POINT"
tags: ["valor CIF", "EXW", "flete internacional", "seguro de carga"]
---

El valor CIF es una de las bases más importantes para estimar una importación. En términos simples, combina el valor de la mercadería, el flete internacional y el seguro.

## Qué significa CIF

CIF viene de *Cost, Insurance and Freight*. En español: costo, seguro y flete.

La fórmula general es:

```text
Valor CIF = Valor EXW + Flete internacional + Seguro
```

## Diferencia entre EXW y CIF

El valor EXW representa el precio de la mercadería en origen, sin sumar el transporte internacional ni el seguro.

El valor CIF agrega esos costos logísticos y se usa como referencia para calcular varios conceptos aduaneros.

## Ejemplo simple

Si una compra tiene estos valores:

```text
EXW: USD 10.000
Flete: USD 1.000
Seguro: USD 100
```

Entonces:

```text
CIF = 10.000 + 1.000 + 100 = USD 11.100
```

Sobre esa base se estiman derechos, tasas e impuestos según el producto y su clasificación.

## Por qué importa para tus márgenes

Un error en el flete o en el seguro puede cambiar toda la estimación. Si calculás impuestos solo sobre el EXW, probablemente estés subestimando el costo real.

Para ver cómo impacta en la rentabilidad, también podés leer la guía sobre [margen bruto y margen neto](/blog/diferencia-margen-bruto-neto).

## Cómo calcularlo rápido

Necesitás tener al menos:

- Valor EXW del proveedor.
- Costo de flete internacional.
- Seguro de carga.
- Tipo de producto o código NCM.
- Tipo de cambio de referencia para tu análisis.

Con esos datos podés usar la [calculadora de importaciones](/calculadora) para obtener una estimación del costo total.
